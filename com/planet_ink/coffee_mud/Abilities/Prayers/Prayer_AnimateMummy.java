...
public class Prayer_AnimateMummy extends Prayer
{
	...
	private final static String localizedName = CMLib.lang()._("Animate Mummy");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget, boolean auto, int asLevel)
	{
		...
		{
			mob.tell(_("This creature is too weak to create a mummy from."));
			return false;
		}
		...
		if(success)
		{
			final CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto),auto?"":_("^S<S-NAME> @x1 to animate <T-NAMESELF> as a mummy.^?",prayForWord(mob)));
			if(mob.location().okMessage(mob,msg))
			{
				mob.location().send(mob,msg);
				final MOB newMOB=CMClass.getMOB("GenUndead");
				newMOB.setName(_("@x1 mummy",race));
				newMOB.setDescription(description);
				newMOB.setDisplayText(_("@x1 mummy is here",race));

...
public class Trap_Popper extends StdTrap
{
	...
	private final static String localizedName = CMLib.lang()._("popping noise");
	...
	public void spring(MOB target)
	{
		if((target!=invoker())&&(target.location()!=null))
		{
			...
			if(target.location().show(target,target,this,CMMsg.MASK_ALWAYS|CMMsg.MSG_NOISE,_("<S-NAME> set(s) off a **POP** trap!")))
			{
				super.spring(target);
				...
				for(final Enumeration e=A.getMetroMap();e.hasMoreElements();)
				{
					...
					if(R!=target.location())
						R.showHappens(CMMsg.MASK_ALWAYS|CMMsg.MSG_NOISE,_("You hear a loud **POP** coming from somewhere."));
				}
